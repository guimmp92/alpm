...
static gboolean
pk_backend_resolve_thread (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	gchar **packages;
	const gchar *package;
	GError *error = NULL;
	...
	for (i=0; i < len(lpackages); i++) {
		package = package_ids[i];
		if (pk_backend_cancelled (self)) {
			break;
		}

		/* find a package with the given id or name */
		if (pk_package_id_check (package)) {
			if (!pk_backend_resolve_package (self, package,
							 &error)) {
				break;
			}
		} else {
			if (!pk_backend_resolve_name (self, package,
						      &error)) {
				break;
			}
		}
...
void
pk_backend_resolve (PkBackend *self, PkBackendJob *job, PkBitfield filters, gchar **package_ids)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (package_ids != NULL);

	pk_backend_run (self, job, PK_STATUS_ENUM_QUERY, pk_backend_resolve_thread);
}

static gboolean
pk_backend_get_details_thread (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	gchar **packages;
	const gchar *package;
	GError *error = NULL;
	...
	for (i=0; i<len(lpackages); i++) {
		...
		pk_backend_job_details (self, package, licenses->str, group,
					desc, url, size);
...
void
pk_backend_get_details (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (package_ids != NULL);

	pk_backend_run (self, job, PK_STATUS_ENUM_QUERY,
			pk_backend_get_details_thread);
}

static gboolean
pk_backend_get_files_thread (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	gchar **packages;
	guint i;
	guint lpackages;
	...
	for (i=0; i<len(lpackages); i++) {
		...
		pk_backend_job_files (self, package, files->str);
...
void
pk_backend_get_files (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (package_ids != NULL);
