...
static void
pk_backend_get_repo_list_thread (PkBackend *self, GVariant *params,
				 gpointer data)
{
	const alpm_list_t *i;
	GHashTableIter iter;
...
void
pk_backend_get_repo_list (PkBackend *self, PkBackendJob *job, PkBitfield filters)
{
	g_return_if_fail (self != NULL);
...
static void
pk_backend_repo_enable_thread (PkBackend *self, GVariant *params,
			       gpointer data)
{
	const gchar *repo;
...
static void
pk_backend_repo_disable_thread (PkBackend *self, GVariant *params,
				gpointer data)
{
	const alpm_list_t *i;
	const gchar *repo;
...
void
pk_backend_repo_enable (PkBackend *self, PkBackendJob *job, const gchar *repo_id, gboolean enabled)
{
	g_return_if_fail (self != NULL);
	...
	if (enabled) {
		pk_backend_job_thread_create (self,
