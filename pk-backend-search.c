...
pk_backend_search_thread (PkBackendJob *job, GVariant *params, gpointer user_data)
{
	...
	//filters = pk_backend_get_pointer (backend, "filters");
	...
	for (ii=0; ii < g_strv_length(search); i++) {
		gpointer pattern = pattern_func (search[ii], &error);
	...
	if (!skip_local) {
		pk_backend_search_db (backend, localdb, match_func, patterns);
	}
	...
	for (i = alpm_get_syncdbs (alpm); i != NULL; i = i->next) {
		if (pk_backend_cancelled (backend)) {
			break;
		}

		pk_backend_search_db (backend, i->data, match_func, patterns);
	}
...
out:
	...
	return pk_backend_finish (backend, error);
}

void
pk_backend_get_packages (PkBackend *self, PkBackendJob *job, PkBitfield filters)
{
	g_return_if_fail (self != NULL);

	/* provide a dummy needle */
	//pk_backend_set_pointer (backend, "search", g_strsplit ("", ";", 0));

	//pk_backend_set_uint (backend, "search-type", SEARCH_TYPE_ALL);
	pk_backend_run (self, job, PK_STATUS_ENUM_QUERY, pk_backend_search_thread);
}

void
pk_backend_search_details (PkBackend *self, PkBackendJob *job, PkBitfield filters, gchar **values)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (values != NULL);

	//pk_backend_set_pointer (backend, "search-type", SEARCH_TYPE_DETAILS);
	pk_backend_run (self, job, PK_STATUS_ENUM_QUERY, pk_backend_search_thread);
}

void
pk_backend_search_files (PkBackend *self, PkBackendJob *job, PkBitfield filters, gchar **values)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (values != NULL);
	...
	//pk_backend_set_pointer (backend, "filters", filters);
...
void
pk_backend_search_groups (PkBackend *self, PkBackendJob *job, PkBitfield filters, gchar **values)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (values != NULL);

	//pk_backend_set_pointer (backend, "search-type", SEARCH_TYPE_GROUP);
	pk_backend_run (self, job, PK_STATUS_ENUM_QUERY, pk_backend_search_thread);
}

void
pk_backend_search_names (PkBackend *self, PkBackendJob *job, PkBitfield filters, gchar **values)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (values != NULL);

	//pk_backend_set_pointer (backend, "search-type", SEARCH_TYPE_NAME);
	pk_backend_run (self, job, PK_STATUS_ENUM_QUERY, pk_backend_search_thread);
}

void
pk_backend_what_provides (PkBackend *self, PkBackendJob *job, PkBitfield filters,
			  PkProvidesEnum provides, gchar **values)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (values != NULL);

	//pk_backend_set_pointer (backend, "search-type", SEARCH_TYPE_PROVIDES);
	pk_backend_run (self, job, PK_STATUS_ENUM_QUERY, pk_backend_search_thread);
}
