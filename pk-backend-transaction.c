...
pk_backend_transaction_dlcb (const gchar *basename, off_t complete, off_t total)
{
	...
	if (complete == 0) {
		...
		pk_backend_job_set_status (backendjob, PK_STATUS_ENUM_DOWNLOAD);
		pk_backend_transaction_download_start (backend, basename);
	} else if (complete == total) {
		dcomplete += complete;
	}

	pk_backend_job_set_item_progress (backend, basename, PK_STATUS_ENUM_DOWNLOAD,sub_percentage);
	pk_backend_job_set_percentage (backend, percentage);
}
				    ...
				    gint percent, gsize targets, gsize current)
{
	...
	switch (type) {
		...
		case ALPM_PROGRESS_KEYRING_START:
			...
			pk_backend_job_set_item_progress (backend, target, PK_INFO_ENUM_DOWNLOADING, percent);
			pk_backend_job_set_percentage (backendjob, overall / targets);
			recent = percent;
...
pk_backend_output_once (PkBackend *self, alpm_pkg_t *pkg, const gchar *output)
{
	...
	pk_backend_job_message (self, PK_MESSAGE_ENUM_UNKNOWN, "<b>%s</b>\n%s",
