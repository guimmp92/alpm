...
static gboolean
pk_backend_transaction_sync_targets (PkBackend *self, GError **error, gchar **package_ids)
{
	guint i;
	guint lpackages;
	gchar *apackage;
	...
	lpackages = g_strv_length (package_ids);

	for (i=0; i<lpackages;i++) {
		apackage = package_ids[i];
		gchar **package = pk_package_id_split (package);
...
static gboolean
pk_backend_download_packages_thread (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	...
	if (pk_backend_transaction_initialize (self, flags, &error) &&
	    pk_backend_transaction_sync_targets (self, &error, package_ids) &&
...
void
pk_backend_download_packages (PkBackend *self, PkBackendJob *job, gchar **package_ids,
			      const gchar *directory)
{
...
static gboolean
pk_backend_simulate_install_packages_thread (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	GError *error = NULL;
...
static gboolean
pk_backend_install_packages_thread (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
...
void
pk_backend_simulate_install_packages (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (package_ids != NULL);
...
void
pk_backend_install_packages (PkBackend *self, PkBackendJob *job, PkBitfield transaction_flags,
			     gchar **package_ids)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (package_ids != NULL);
...
static gboolean
pk_backend_update_packages_thread (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	gboolean only_trusted;
	const alpm_list_t *i;
...
void
pk_backend_simulate_update_packages (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (package_ids != NULL);
...
void
pk_backend_update_packages (PkBackend *self, PkBackendJob *job, PkBitfield transaction_flags,
			    gchar **package_ids)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (package_ids != NULL);
