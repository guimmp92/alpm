...
static gboolean
pk_backend_get_depends_thread (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	gchar **packages;
	...
	for (ii=0; ii<lpackages; ii++) {
		apackage = package_ids[ii];
		alpm_pkg_t *pkg;
...
static gboolean
pk_backend_get_requires_thread (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	gchar **packages;
	alpm_list_t *i, *pkgs = NULL;
	GError *error = NULL;
...
void
pk_backend_get_depends (PkBackend *self, PkBackendJob *job, PkBitfield filters,
...
void
pk_backend_get_requires (PkBackend *self, PkBackendJob *job, PkBitfield filters,
			 gchar **package_ids, gboolean recursive)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (package_ids != NULL);
