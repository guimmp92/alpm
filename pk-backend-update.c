...
static gboolean
pk_backend_get_update_detail_thread (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
	gchar **packages;
	GError *error = NULL;
	guint i;
	...
	for (i=0; i<lpackages; i++) {
		...
		pkg = pk_backend_find_pkg (self, apackage, &error);
		if (pkg == NULL) {
			break;
		}
		...
		pk_backend_job_update_detail (self, apackage, upgrades,
...
void
pk_backend_get_update_detail (PkBackend *self, PkBackendJob *job, gchar **package_ids)
{
...
static gboolean
pk_backend_get_updates_thread (PkBackend *self, PkBackendJob *job)
{
	struct stat cache;
	time_t one_hour_ago;
...
void
pk_backend_get_updates (PkBackend *self, PkBackendJob *job, PkBitfield filters)
{
	g_return_if_fail (self != NULL);
...
static gboolean
pk_backend_refresh_cache_thread (PkBackend *self, PkBackendJob *job)
{
	gint force;
	GError *error = NULL;
...
void
pk_backend_refresh_cache (PkBackend *self, PkBackendJob *job, gboolean force)
{
	g_return_if_fail (self != NULL);
